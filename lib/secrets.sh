...
TWPR_cmd_secret_rotate() {
  TWPR_require_root
  TWPR_load_state
  local new_secret choice=""
  new_secret="$(TWPR_gen_secret)"
  TWPR_info "Новый secret: ${C_BOLD}${new_secret}${C_RESET}"
  TWPR_ask_yn choice "Записать в profiles.json и перезапустить" "Y"
  [[ "$choice" == "yes" ]] || return 0
...
TWPR_cmd_secret_add() {
  ...
  TWPR_ask name "Имя профиля" "user$(date +%s | tail -c 5)"
  secret="$(TWPR_gen_secret)"
  TWPR_ask backend "Backend loopback" "127.0.0.1:2398"
